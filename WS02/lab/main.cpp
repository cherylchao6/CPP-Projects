...
int main() {
  if (readTesters()) {
    ...
    if (cnt) {
      ...
      cout << "File records and number of records do not match!"
           << endl; // this is the correct output
    }
    fclose(fptr);
    ...
    if (cnt) {
      ...
      cout << "This should not have failed, keep working on the workshop!"
           << endl;
    }
    fclose(fptr);
  }
...
bool readTesters() {
  ...
  if (passed && read(str, fptr) &&
      strcmp(str, "Applied Problem Solving") == 0) {
    cout << "bool read(double& value, FILE* fptr) test passed" << endl;
  } else {
    cout << "bool read(double& value, FILE* fptr) test failed" << endl;
    passed = false;
