...
int main() {
  ...
  printHeader(cout, "[#2] Bank Account (I/O)",
              "John Evans\n100.25\n2022 12 6\n01234\n002\n0012345");
  cin >> bankAcct;
  cout << bankAcct << endl << endl;
  ...
  SavingsAccount savAcct("Frank Wilson", savDate, savDep, 99.5f, 0.055f,
                         interestDate);

  cout << savAcct << endl;

  interestDate = Date(interestDate.getDay(), interestDate.getMonth() + 1,
                      interestDate.getYear());
...
void printHeader(ostream &out, const char *title, const char *values) {
  if (values) {
    out << "Enter the following values when prompted: " << endl
        << values << endl
        << "---------------------------" << endl;
  }
